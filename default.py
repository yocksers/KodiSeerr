...
def do_request(media_type, id):
    ...
    seasons_to_request = []
    if media_type == "tv":
        tv_cache_key = f"details_tv_{id}"
        tv_data = get_cached(tv_cache_key)
        if not tv_data:
            tv_data = api_client.client.api_request(f"/tv/{id}")
            if tv_data:
                set_cached(tv_cache_key, tv_data)

        current_season = args.get("season")
        ...
        if chosen == "Request all seasons":
            ...
            snum = int(chosen.replace("Season ", ""))
            seasons_to_request = [snum]
    ...
    if addon.getSettingBool('confirm_before_request'):
        title_data_key = f"details_{media_type}_{id}"
        title_data = get_cached(title_data_key)
        if not title_data:
            title_data = api_client.client.api_request(f"/{media_type}/{id}")
            if title_data:
                set_cached(title_data_key, title_data)
...
def show_requests(data, mode, current_page):
    ...
    for item in items:
        ...
        cache_key = f"details_{media_type}_{id}"
        mediaData = get_cached(cache_key)
        if not mediaData:
            mediaData = api_client.client.api_request(f"/{media_type}/{id}", params={})
            if mediaData:
                set_cached(cache_key, mediaData)
        if not mediaData:
            continue
...
def list_recently_added():
    ...
    cache_key_movies = f"recently_added_movies_{page}"
    recent_movies = get_cached(cache_key_movies)
    if not recent_movies:
        recent_movies = api_client.client.api_request("/discover/movies", params={"sortBy": "mediaAdded", "page": page})
        if recent_movies:
            set_cached(cache_key_movies, recent_movies)

    cache_key_tv = f"recently_added_tv_{page}"
    recent_tv = get_cached(cache_key_tv)
    if not recent_tv:
        recent_tv = api_client.client.api_request("/discover/tv", params={"sortBy": "mediaAdded", "page": page})
        if recent_tv:
            set_cached(cache_key_tv, recent_tv)
